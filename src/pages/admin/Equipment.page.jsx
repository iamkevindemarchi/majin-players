...
// Api
import { EQUIPMENT_API, IMAGES_API } from "../../api";
...
const Equipment = () => {
    ...
    const isEditMode = equipmentId ? true : false;
    ...
    async function getEquipmentHandler() {
        ...
        const res = await EQUIPMENT_API.get(equipmentId);
    ...
    function goBackToEquipmentsHandler() {
        navigate(`/admin/equipment`);
    }

    const goBackBtn = isEditMode ? (
        <GoBtn theme={theme} page="/admin/equipment" label="Torna indietro" />
    ) : (
    ...
    function validateForm() {
        ...
        if (!isNameValid || !isImgValid) return false;
        else return true;
    }
    ...
    async function createHandler() {
        ...
        if (isFormValid) {
            ...
            const res = await EQUIPMENT_API.create(data);
            ...
            if (res) {
                ...
                if (imageRes) {
                    ...
                    navigate(`/admin/equipment/edit/${res.id}`);
                } else
                    activeSnackbar(
                        "error",
                ...
                activeSnackbar("error", "Impossibile creare l'equipaggiamento");
    ...
    async function editHandler() {
        ...
        if (isFormValid) {
            ...
            const res = await EQUIPMENT_API.update(data, equipmentId);
            if (res) {
                if (isImageChanged) {
    ...
    const form = (
        <form className="flex flex-col gap-5 w-full justify-center desktop:px-60">
            ...
            <div className="flex flex-row justify-center gap-5">
                ...
                <Button variant="cancel" onClick={goBackToEquipmentsHandler}>
                    <CancelIcon />
                    Anulla
                </Button>
            </div>
    ...
    useEffect(() => {
        equipmentId && getEquipmentHandler();
